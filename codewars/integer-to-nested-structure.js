...
function decode(n) {
   ...
   *
   */
  function mapRule(aryItem) {
    const { length } = aryItem;
    if (length === 0) {
      return 2;
    }
    ...
    switch (length) {
      case 0:
        return 2;
      case 1:
        const first = aryItem[0];
        if (first.length === 0) {
          return findIndexPrime(mapRule(first));
        } else {
          if (first.length === 1) {
            return findIndexPrime(mapRule(first));
          }

          const re = first.reduce((p, c) => {
            const r = mapRule(c);
            return r * p;
          }, 1);
          return findIndexPrime(findIndexPrime(re));
        }
      default:
        return findIndexPrime(
          aryItem.reduce((item, i) => {
            const r = mapRule(i);
            return item * r;
          }, 1)
        );
  ...
  const result = arry.reduce((item, i) => {
    const r = mapRule(i);
    return item * r;
  }, 1);
  return result;
...
assert(decode(encode(10005)) === 10005);
