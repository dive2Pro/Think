...
describe('Atom class specifications (using methane)', () => {
    ...
    it('toString() implementation: basics', () => {
...
describe('Invalid mutations and additions', () => {
    ...
    config.forEach(([descript, branch, bonds, mutadd, inp]) => {
        ...
        var func;
        if (mutadd === 'mutate')
            func = () => m.mutate(inp).closer();
        else if (mutadd === 'add')
            func = () => m.add(inp).closer();
        else if (mutadd === 'mutadd')
            func = () => m.mutate(inp[0]).add(inp[1]).closer();
        else
            throw new Error('Wrong configuration of the tests');
        it(descript, () => {
